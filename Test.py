...
parser.add_argument('--arch', type=str, default='STAM', choices=['ResNet50', 'STE'])
...
parser.add_argument('--dataset', type=str, default='prid', choices=['ilids','prid'])
...
def main():
    ...
    if args_.test_sampler == 'dense':
        ...
        queryloader = DataLoader(
            Video_Event_Dataset(dataset.query, seq_len=args_.seq_len, sample=args_.test_sampler, transform=transform_test,
        ...
        galleryloader = DataLoader(
            Video_Event_Dataset(dataset.gallery, seq_len=args_.seq_len, sample=args_.test_sampler, transform=transform_test,
                         max_seq_len=cfg.DATASETS.TEST_MAX_SEQ_NUM, dataset_name=cfg.DATASETS.NAME),
            batch_size=1 , shuffle=False, num_workers=cfg.DATALOADER.NUM_WORKERS,
            pin_memory=pin_memory, drop_last=False,
        )
    ...
    #model_E2VID = load_E2VID("e2vid_utils/E2VID_lightweight.pth.tar", device='cpu')
...
def test(model, queryloader, galleryloader, pool, use_gpu, dataset, ranks=[1, 5, 10, 20]):
    ...
    with torch.no_grad():
        ...
        cmc, be_mAP, q_g_dist = evaluate_reranking(qf, q_pids, q_camids, gf, g_pids, g_camids, ranks, args_.test_distance)
        return cmc, be_mAP, q_g_dist

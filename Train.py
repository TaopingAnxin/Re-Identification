...
from event_loader import Video_Event_Dataset
import torch
...
parser.add_argument('--arch', type=str, default='ResNet50', choices=['ResNet50', 'STE'])
parser.add_argument('--train_sampler', type=str, default='Random_interval', help='train sampler',
                    choices=['Random_interval', 'Random_choice'])
...
parser.add_argument('--dataset', type=str, default='prid', choices=['ilids', 'prid', 'mars'])
...
def main():
    ...
    model = models.init_model(name=args_.arch, num_classes=dataset.num_train_pids,
    ...
    trainloader = DataLoader(
        Video_Event_Dataset(dataset.train, seq_len=args_.seq_len, sample=args_.train_sampler, transform=transform_train,
    ...
    if args_.test_sampler == 'dense':
        ...
        queryloader = DataLoader(
            Video_Event_Dataset(dataset.query, seq_len=args_.seq_len, sample=args_.test_sampler,
        ...
        galleryloader = DataLoader(
            Video_Event_Dataset(dataset.gallery, seq_len=args_.seq_len, sample=args_.test_sampler,
                                transform=transform_test,
                                max_seq_len=cfg.DATASETS.TEST_MAX_SEQ_NUM, dataset_name=cfg.DATASETS.NAME),
            batch_size=1, shuffle=False, num_workers=cfg.DATALOADER.NUM_WORKERS,
            pin_memory=pin_memory, drop_last=False,
    ...
    for epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCHS):
        ...
        if cfg.SOLVER.EVAL_PERIOD > 0 and (
                (epoch + 1) % cfg.SOLVER.EVAL_PERIOD == 0 or (epoch + 1) == cfg.SOLVER.MAX_EPOCHS) or epoch == 0:
            print("==> Test")
            _, metrics, be_mAP = test(model, queryloader, galleryloader, use_gpu)
            torch.cuda.empty_cache()
            rank1 = metrics[0]
            state_dict = model.state_dict()
            torch.save(state_dict, osp.join(cfg.OUTPUT_DIR,
                                            "mAP_" + str(be_mAP) + "rank1_" + str(rank1) + '_checkpoint_ep' + str(
                                                epoch + 1) + '.pth'))
...
def train(model, trainloader, xent, tent, optimizer, use_gpu):
    ...
    for batch_idx, (imgs, pids, _, _) in enumerate(trainloader):
        ...
        category_scores, BN_feature_list = model(imgs)
        xent_loss = sum(xent(score, pids) for score in category_scores)
        tent_loss = sum(tent(feature, pids) for feature in BN_feature_list)
        ...
        xent_losses.update(xent_loss.item(), imgs.size(0))
        tent_losses.update(tent_loss.item(), imgs.size(0))
        losses.update(loss.item(), imgs.size(0))

    print("Batch {}/{}\t Total Loss {:.6f} ({:.6f}) Xent Loss {:.6f} ({:.6f}), Tent Loss {:.6f} ({:.6f})".format(
    ...
    with torch.no_grad():
        ...
        for batch_idx, (imgs, pids, camids, img_path) in enumerate(tqdm(queryloader)):
            ...
            if len(imgs.size()) == 6:
                ...
                method = None
            # features, pids, camids, _ = model(imgs, pids, camids)
        ...
        with open('query_pathes.json', 'w') as f:
            json.dump(query_pathes, f)

        with open('query_pathes.json', 'r') as f:
            loaded_query_pathes = json.load(f)
        ...
        with open('gallery_pathes.json', 'w') as f:
            json.dump(gallery_pathes, f)

        with open('gallery_pathes.json', 'r') as f:
            loaded_gallery_pathes = json.load(f)

        print("Extracted features for gallery set, obtained {}-by-{} matrix".format(gf.size(0), gf.size(1)))
        print("Computing distance matrix")

        be_cmc, be_mAP, metrics = evaluate_reranking(qf, q_pids, q_camids, gf, g_pids, g_camids, ranks,
                                                     args_.test_distance)
        return metrics, be_cmc, be_mAP

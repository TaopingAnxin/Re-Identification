...
class TripletLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        for i in range(n):
            ...
            #dist_an.append(dist[i][mask[i] == 0].min(dist[i]).unsqueeze(0))
            dist_an.append(dist[i][mask[i] == 0].min().unsqueeze(0) if (mask[i] == 0).sum() > 0 else torch.zeros(1))
        ...
        if torch.cuda.is_available():
            dist_an = dist_an.cuda()
            dist_ap = dist_ap.cuda()
            y = y.cuda()
